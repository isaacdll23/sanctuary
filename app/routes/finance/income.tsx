  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Income Stats */}
        <div className="bg-slate-800/80 backdrop-blur-md border border-slate-700 rounded-2xl shadow-xl p-6 mb-8 w-full max-w-2xl mx-auto">
          {loaderData.annualGrossIncome ? (
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="bg-slate-700/50 rounded-xl p-4">
                  <p className="text-slate-400 text-sm mb-1">Annual Gross Income</p>
                  <p className="text-2xl font-semibold text-white">${loaderData.annualGrossIncome.toLocaleString()}</p>
                </div>
                <div className="bg-slate-700/50 rounded-xl p-4">
                  <p className="text-slate-400 text-sm mb-1">Tax Deduction Rate</p>
                  <p className="text-2xl font-semibold text-white">{loaderData.taxDeductionPercentage}%</p>
                </div>
              </div>

              <div className="bg-slate-700/30 rounded-xl p-5">
                <h2 className="text-xl font-semibold mb-4 text-slate-200">Estimated Breakdowns</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-slate-400 text-sm">Annual Tax</p>
                    <p className="text-lg font-medium text-rose-400">
                      ${((loaderData.annualGrossIncome * loaderData.taxDeductionPercentage) / 100).toLocaleString(undefined, {maximumFractionDigits: 2})}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-400 text-sm">Monthly Tax</p>
                    <p className="text-lg font-medium text-rose-400">
                      ${((loaderData.annualGrossIncome * loaderData.taxDeductionPercentage) / 1200).toLocaleString(undefined, {maximumFractionDigits: 2})}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-400 text-sm">Annual Net Income</p>
                    <p className="text-lg font-medium text-emerald-400">
                      ${((loaderData.annualGrossIncome * (100 - loaderData.taxDeductionPercentage)) / 100).toLocaleString(undefined, {maximumFractionDigits: 2})}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-400 text-sm">Monthly Net Income</p>
                    <p className="text-lg font-medium text-emerald-400">
                      ${((loaderData.annualGrossIncome * (100 - loaderData.taxDeductionPercentage)) / 1200).toLocaleString(undefined, {maximumFractionDigits: 2})}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="w-full max-w-2xl mx-auto">
          <div className="bg-slate-800/80 backdrop-blur-md border border-slate-700 rounded-2xl shadow-xl p-6">
            <h2 className="text-xl font-semibold mb-6 text-slate-200">Update Income Details</h2>
            
            <fetcher.Form method="post" className="space-y-6">
              <div>
                <label htmlFor="annualGrossIncome" className="block text-sm font-medium text-slate-300 mb-1">
                  Annual Gross Income
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-4 py-3 bg-slate-700/50 border border-slate-600 text-slate-100 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors"
                    required
                  />
                </div>
              </div>
              ...
              <div>
                <label htmlFor="taxDeductionPercentage" className="block text-sm font-medium text-slate-300 mb-1">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-4 pr-8 py-3 bg-slate-700/50 border border-slate-600 text-slate-100 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors"
                    required
                  />
                ...
                <p className="mt-2 text-xs text-slate-400">Enter the percentage of your income that goes to taxes.</p>
              </div>
