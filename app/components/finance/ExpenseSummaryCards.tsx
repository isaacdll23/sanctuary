...
export default function ExpenseSummaryCards({
  ...
  return (
    <section className="mb-4 rounded-xl border border-gray-300 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div className="mb-3 flex items-center gap-2">
        <ArrowPathIcon className="h-5 w-5 text-gray-600 dark:text-gray-400" />
        <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Recurring cost snapshot</h2>
      </div>
      <dl className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {metrics.map((metric) => (
          <div key={metric.label} className="rounded-lg bg-gray-100 px-3 py-2 dark:bg-gray-700/60">
